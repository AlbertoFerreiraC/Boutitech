...
public class Buscador extends javax.swing.JFrame {
    ...
    public Buscador(String _sql, String[] nomColu, int canColu, Object[] retorno) {
        ...
        query = "select idproveedor, nombre, direccion from proveedor";
        nom = nomColu;
        col = canColu;
        tfParam = retorno;
    ...
    private void Buscado() {
        try {
            ...
            conn.sentencia = conn.conectar.createStatement();

            conn.resultado = conn.sentencia.executeQuery(query + " upper('%" + this.txtValorBusqueda.getText() + "%')");
            val = new ArrayList<>();
